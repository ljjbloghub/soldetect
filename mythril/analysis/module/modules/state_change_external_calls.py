...
class StateChangeCallsAnnotation(StateAnnotation):
    ...
    def get_issue(
        ...
        description_head = "  外部调用后{}合约状态。".format(
            read_or_write
        )
        description_tail = ("  重入攻击（又称递归调用攻击）可以接管控制流，智能合约在函数的第一次调用完成之前回调到调用\n合约中。这可能导致函数间的调用以不被希望的方式进行交互" )

        solutions=["1）在需要进行以太币交易时，使用transfer()或send()函数，此类函数只会使用2300个gas用于处理转币操作；",
