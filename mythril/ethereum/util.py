...
def get_solc_json(file, solc_binary="solc", solc_settings_json=None):
    """
    #对solidit进行反汇编处理，返回包含抽象语法树ast，opcode的json文件
    :param file:
    :param solc_binary:
    ...
    #--optimize 进行优化；--allow-paths 限制访问目录；
    #--standard-json调用SOLC，它就将标准的JSON输入，并返回JSON输出
            ...
            "outputSelection": {
                "*": {
                    ...
                    "*": [
                        ...
                        "evm.methodIdentifiers",#合约中函数的哈希值
                    ],
                }
            }
        }
...
def solc_exists(version):
    ...
    if sys.version_info[1] >= 6:
        ...
        if not solc.main.is_solc_available():
            ...
            solc_binaries = [
                os.path.join(
                    os.environ.get("HOME", str(Path.home())),#返回家目录
