...
class LaserEVM:
    """The LASER EVM.
    挖掘控制流程图，获取符号执行数据并进行漏洞模块检测
    
    """

    def __init__(
        self,
        dynamic_loader=None,#DynLoader()动态加载，用于获取链上存储数据和依赖项的动态加载程序逻辑
        max_depth=float("inf"),
        execution_timeout=60,
        create_timeout=10,
        strategy=DepthFirstSearchStrategy,#默认为bfs
        transaction_count=2,
        ...
        self.total_states = 0
        self.dynamic_loader = dynamic_loader#用于获取链上存储数据和依赖项的动态加载程序逻辑

        self.work_list = []  # type: List[GlobalState]
        
        self.strategy = strategy(self.work_list, max_depth)#初始化遍历搜索策略 worklist
        ...
        self.requires_statespace = requires_statespace#true
        ...
        for op, _, _, _ in OPCODES.values():#遍历操作码 0x00: ("STOP", 0, 0, 0),op=STOP
            self.instr_pre_hook[op] = []
            self.instr_post_hook[op] = []
        log.info("LASER EVM initialized with dynamic loader: " + str(dynamic_loader))

    def extend_strategy(self, extension: ABCMeta, *args) -> None:
        #extension其实为mythril.laser.ethereum.strategy.extensions.bounded_loops.BoundedLoopsStrategy
    ...
    def sym_exec(
        ...
        1.分析预配置信息（pre_configuration），需要提供world_state 和target_address
        2.执行合约的创建代码，需要提供creation code和合约名字
        ...
        time_handler.start_execution(self.execution_timeout)#定义执行开始时间
        self.time = datetime.now()

        if scratch_mode:

            #根据creation_code，及open state执行合同创建事务,返回Account类
            created_account = execute_contract_creation(
                self, creation_code, contract_name, world_state=world_state
            )

            self._execute_transactions(created_account.address)#address 由symbol_factory.BitVecVal(address, 256)生成

        
        """
        self.register_laser_hooks(hook_type, func)->self._xxx_sym_exec_hooks.append(hook)
        hook为装饰器返回的加载插件功能中的函数（SymExecWrapper类封装）
    ...
    def exec(self, create=False, track_gas=False) -> Optional[List[GlobalState]]:
        ...
        for global_state in self.strategy:#按照策略从work_list中返回global_state
    ...
    def register_hooks(self, hook_type: str, hook_dict: Dict[str, List[Callable]]):
        ...
        op_code:CREATE
        funcs:[ pre_hooks=['CALL', 'DELEGATECALL', 'CALLCODE', 'SSTORE', 'SLOAD', 'CREATE', 'CREATE2'] 
            post_hooks=[]
        entrypoint:{'BLOCKHASH': [<DetectionModule name=Control flow depends on a predictable environment variable swc_id=116 120 
                pre_hooks=['JUMPI', 'BLOCKHASH'] 
                post_hooks=['BLOCKHASH', 'COINBASE', 'GASLIMIT', 'TIMESTAMP', 'NUMBER'],..
        """
        if hook_type == "pre":
            entrypoint = self.pre_hooks#DefaultDict[str, List[Callable]]
        elif hook_type == "post":
    ...
    def laser_hook(self, hook_type: str) -> Callable:
        ...
        def hook_decorator(func: Callable):
            ...
            return func#SymExecWrapper类中加载插件处通过装饰器返回可调用函数stop_sym_exec_hook()等
