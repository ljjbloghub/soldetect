...
class SignatureDB(object, metaclass=Singleton):
    ...
    def add(self, byte_sig: str, text_sig: str) -> None:#byte_sig->sign ,text_sig->name
        """
    ...
    def import_solidity_file(
        ...
        """通过solc命令进行反汇编并通过sqlite存储进行反汇编sol文件中的函数和对应签名
        ...
        :return:
        """
        solc_json = get_solc_json(file_path, solc_binary, solc_settings_json)#！！！！通过solc命令进行反汇编，得到抽象语法树ast,opcode等，结果为json格式

        for contract in solc_json["contracts"][file_path].values():
            for name, hash_ in contract["evm"]["methodIdentifiers"].items():#返回函数名和对应hash值
                sig = "0x{}".format(hash_)
                if sig in self.solidity_sigs:#为defalutlist
